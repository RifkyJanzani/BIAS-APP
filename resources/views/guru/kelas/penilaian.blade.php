        ...
        <form action="{{ $penilaian->isNotEmpty() ? route('update.penilaian', $penilaian->first()->id) : route('submit.penilaian') }}" method="POST" enctype="multipart/form-data" id="penilaianForm">
            ...
            <div class="p-3 border rounded shadow-sm">
                ...
                <div class="m-3 p-3 border rounded shadow-sm">
                    <label for="capaian_{{ $item['id'] }}" class="form-label">{{ $item['pernyataan'] }}</label>
                    <div id="capaian_{{ $item['id'] }}" class="d-flex flex-wrap flex-column flex-sm-row justify-content-sm-around justify-content-start gap-sm-0 gap-2">
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="belumMuncul_{{ $item['id'] }}">
                                Belum Muncul
                            </label>
                        </div>
                        <div class="form-check">
                            ...
                            {{ $penilaianItem ? ($penilaianItem->capaian == '0' ? '' : 'checked') : '' }}>
                            <label class="form-check-label" for="muncul_{{ $item['id'] }}">
                                Muncul
                            </label>
                        </div>
                    </div>
